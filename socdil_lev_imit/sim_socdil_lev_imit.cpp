...
std::vector<SimParameter> Sim_socdil_lev_imit::parameters()
{
	return {e, d, zz, Temp};
}

uint32_t Sim_socdil_lev_imit::animationDelay() const
{
	return 0;		/* UC: returns the animation delay of the simulation(how long it waits before displaying the next state, in ms) */
}

uint32_t Sim_socdil_lev_imit::renderFrameSkip() const
{
	return 50;		/* UC: returns the frame skip of the simulation(how many steps are done in the background) */
}

size_t Sim_socdil_lev_imit::displayWidth() const
{
	return 150;
}

void Sim_socdil_lev_imit::step()		/* defines a simulation step */
{
...
void Sim_socdil_lev_imit::reset()		/* creates a specific strategy distribution across the grid */
{
...
size_t Sim_socdil_lev_imit::stepTargetNumber() const
{
